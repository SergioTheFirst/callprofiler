...
Для контактов БЕЗ display_name (только номер) ищет имя в первых 10 сегментах
каждого звонка (оба канала [me] и [s2] — роли часто перепутаны).
...
Имена владельца исключаются:
  Сергей, Станиславович, Серёжа, Серёж, Серёга, Серж, Серёг, Сереж, Медведев

Confidence:
  - 1 звонок с именем → "medium"
  - 2+ звонков с ТЕМ ЖЕ именем → "high"

Результат записывается в contacts: guessed_name, guess_source='regex', guess_confidence.

CLI: python -m callprofiler extract-names --user <user_id>
"""
...
_STOPWORDS: frozenset[str] = frozenset({
    ...
    # должности / роли
    "менеджер", "специалист", "оператор", "директор", "начальник",
    "доктор", "коллега", "клиент", "секретарь", "курьер",
    "водитель", "мастер", "администратор",
    # организации
    "компания", "отдел", "магазин", "офис", "фирма", "салон", "аптека",
    "склад", "служба", "банк",
    # родственные и бытовые обозначения
    "брат", "сестра", "мама", "папа", "дядя", "тётя", "тетя",
    "бабушка", "дедушка", "сын", "дочь", "муж", "жена",
    "дед", "батя", "отец", "мать", "внук", "внучка",
    # общие обращения (не имена собственные)
    "дорогой", "уважаемый", "любимый", "милый", "подруга", "друг",
    "гражданин", "гражданка", "господин", "госпожа",
    # не-имена из речи
    "человек", "девушка", "молодой", "женщина", "мужчина",
})
