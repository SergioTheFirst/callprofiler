...
def build_yearly_summary_prompt(
    ...
    user = (
        f"Год: {year}\n\n"
        f"Главы года:\n"
        f"{json.dumps(chapters_slim, ensure_ascii=False, indent=2)[:5000]}\n\n"
        f"Главные арки:\n"
        f"{json.dumps(arcs_slim, ensure_ascii=False, indent=2)[:3000]}\n\n"
        f"Ключевые персонажи/места:\n"
        f"{json.dumps(ents_slim, ensure_ascii=False, indent=2)[:1500]}\n\n"
        f"Напиши годовой итог: 3-5 абзацев, без подзаголовков, без морали."
    )
    return [
