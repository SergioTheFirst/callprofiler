...
- Никакой беллетристики на пустом месте: не додумывай мотивы, если их нет в
  материале. Если факт неполон — так и оставь, не достраивай.
"""
...
Строго верни ОДИН JSON-объект, без markdown-оберток, без пояснений:
{
  ...
  "insight": "1 фраза: чем эта сцена может быть интересна читателю книги;
               оставь пустой строкой, если сцена рутинная"
}
...
Строго JSON:
{
  "prose": "3-5 абзацев. Как персонаж появляется в жизни владельца, что их
            ...
            Никакого психологизирования на пустом месте.",
...
- Длина главы: 2500-4500 слов. Лучше ближе к 3500. Это не инструкция «лей
  воду» — это просьба развернуть материал, а не бежать по нему.
...
- Покажи минимум одну сцену с эмпатией к собеседнику, даже если ситуация
  была конфликтной.
- Имена — канонические (Василий, Пётр Иванович, Екатерина). Клички/
  уменьшительные — только если это бытовая речь в цитате.
...
- Объём — примерно как в черновике (+/- 15%). Если черновик короче 2500 слов,
  можно расширить до 3000-3500, но только если есть чем.
