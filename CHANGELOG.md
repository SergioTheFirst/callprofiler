...
### Fixed — облачный прогон: extras не отсекал ML-стек + скрытая порча БД при restore (2026-08-21)

**Контекст:** первый реальный запуск autonomous routine (`docs/routines/continue-sintezdiharea.md`)
в облаке. Ориентация по критическому пути (§9.1) указывала на T-07 следующей задачей, но
верификация окружения ДО начала T-07 вскрыла, что сама предпосылка «облако — дёшево и без
ML-стека» (2026-08-08) была измерена неверно и на деле ложна. Вместо T-07 эта сессия закрыла
найденные дефекты — ровно то, что требует протокол ПРОВЕРКА.

- **`pip install -e ".[cloud]"` реально ставил ПОЛНЫЙ ML-стек (torch/pyannote.audio/
  faster-whisper/soundfile/librosa, ~150 пакетов), а не «~40МБ без ML» как документировано.**
  Root cause: extras в pip только ДОБАВЛЯЮТ пакеты к обязательным `dependencies` — не могут их
  исключить. torch и весь ML-стек жили в `[project] dependencies` (безусловно), поэтому ЛЮБОЙ
  extra, включая `cloud`, тянул их вместе с базовым списком. Заявленное «1415 passed, 7 skipped,
  0 failed» (2026-08-08) было измерено СИМУЛЯЦИЕЙ отсутствия torch (заглушка импорта), а не
  реальным `pip install -e ".[cloud]"` в чистом окружении — расхождение никогда не всплывало.
  Перепроверено в этой сессии реальной установкой в чистый venv: до фикса — torch стоит,
  150 пакетов; после фикса — torch отсутствует, 39 пакетов.
  **Fix:** `torch`/`faster-whisper`/`pyannote.audio`/`soundfile`/`librosa` вынесены из
  `dependencies` в новый extra `full` (боевой GPU-бокс). `pip install -e ".[cloud]"` теперь
  реально лёгкий; `pip install -e ".[full]"` (README/AGENTS.md/docs/TESTING.md обновлены)
  сохраняет прежнее поведение на боксе/деве.
- **Ещё один слой той же неполной верификации — 14 тестов реально падали в чистом cloud-venv,
  которых симуляция не поймала:**
  - `test_dashboard_import.py::test_path_traversal_confined_to_incoming_dir` — реальный, не
    тестовый дефект: `_resolve_incoming_dst` (dashboard/tools.py, M5 security-sensitive) резал
    path traversal через `Path(filename).name`, а это платформозависимо — backslash является
    разделителем пути только в `WindowsPath`; на POSIX `Path("..\\..\\evil.mp3").name` возвращает
    строку БЕЗ ИЗМЕНЕНИЙ (traversal не реализуется на POSIX благодаря отсутствию `\` как
    разделителя ФС, но санитайзер молча пропускает подозрительное имя как есть — devare/CI на
    Linux, бокс на Windows, поведение расходится незаметно). **Fix:** нормализация `\` → `/`
    ПЕРЕД `Path(...).name` — одна строка, платформонезависимо, не ослабляет боевую защиту.
  - `test_event_bus.py` (6) + `test_dashboard_tools.py::TestAsyncOperations` (4) +
    `test_dashboard_request_scoped.py::TestStreamingUpload` (2) — `pytest-asyncio` нигде не
    объявлен зависимостью, хотя `@pytest.mark.asyncio` используется в 3 файлах. В чистом venv
    падало «async def functions are not natively supported». **Fix:** `pytest-asyncio>=0.24` в
    extras `dev`/`cloud`, `asyncio_mode = "strict"` + регистрация маркеров в `pyproject.toml`
    (снимает заодно `PytestUnknownMarkWarning` на `unit`/`asyncio`).
- **`test_backup.py::test_restore_overwrite_snapshots_current_state_first` — НЕ тестовая
  проблема, реальная порча данных при restore (T-20, P-OPS-01, ровно тот гейт, который должен
  защищать T-05 migrations).** `restore_backup()` атомарно подменяет основной файл `.db`
  (`os.replace`), но НЕ трогает `-wal`/`-shm` sidecar-файлы старого (перезаписываемого)
  назначения. Если назначение раньше было живой БД в WAL-режиме (штатный режим — каждое
  соединение `ConnectionFactory` включает `journal_mode=WAL`), эти sidecar-файлы содержат кадры
  СТАРЫХ данных; следующее открытие свежевосстановленного файла реплеит их поверх, и
  восстановленные счётчики строк молча откатываются к состоянию ДО restore (воспроизведено:
  ожидали 3 звонка из бэкапа, получили 1 — ровно число из перезаписанной "живой" БД).
  **Fix:** `restore_backup()` удаляет `<to_path>-wal`/`<to_path>-shm` сразу после `os.replace`,
  до верификации счётчиков — восстановленный файл гарантированно открывается со свежим WAL.
  Регресс уже был у существующего теста (не вакуумный: без фикса падает с тем самым мисматчем
  1≠3, с фиксом проходит) — новый тест не понадобился.
- **Обе верифицированные базовые линии (реальный `pip install`, не заглушка, оба venv «с нуля»
  этой сессией):** облако (`.[cloud]`) — **1415 passed, 7 skipped, 0 failed**; полный стек
  (`.[dev,full]`) — **1459 passed, 3 skipped, 0 failed**. Числа совпадают с ранее заявленными —
  но теперь реально измерены той же командой, что выполнит routine, а не симуляцией.
- T-07 (следующая задача критического пути §9.1) не начата в этой сессии — вся её ёмкость ушла
  на закрытие найденных дефектов, что важнее по протоколу ПРОВЕРКА («нашёл — чини сам»).
  `ruff` на изменённых файлах не даёт новых находок сверх известного 163-ledger (`shutil`
  unused-import в `test_backup.py`, например, — существовал до этой сессии, обращение к
  `bk.shutil`, не к своему импорту).
- Файлы: `pyproject.toml`, `README.md`, `AGENTS.md`, `docs/TESTING.md`,
  `src/callprofiler/ops/backup.py`, `src/callprofiler/dashboard/tools.py`.
  Память: `.claude/rules/bugs.md` (обе находки — WAL restore + путь-санитайзер).
...
- Задача: routine раз в 6 часов в облаке при **выключенной** машине владельца. Измерено, а не
  ...
  57 тестов падало**. То есть гейт «suite зелёный» в облаке был бы недостижим.
- Убраны три eager-импорта ML (тот же класс, что P-OPS-03, который T-01 закрыл только в
  `__init__.py` пакета):
  1. `orchestrator` тянул `WhisperRunner` (→ `torch`) на верхнем уровне модуля **даже при
     `asr_backend: gigaam`**, когда Whisper вообще не используется. GigaAM при этом уже
     импортировался лениво — ленивость в одной ветке фабрики и eager в другой.
  2. ASR-runner создавался прямо в `Orchestrator.__init__` → сам факт конструирования
     оркестратора требовал ML-стек. Теперь ленивое свойство с сеттером (тесты подменяют фейком),
     симметрично pyannote, который был ленивым и раньше.
  3. `_ref_fingerprint` жил в `pyannote_runner`, хотя это чистая работа с ФС. Вынесен в
     `artifacts.py::file_fingerprint` — сторож reference-эмбеддинга (T-10) теперь проверяется
     и там, где ML-стека нет. Алиас в `pyannote_runner` сохранён.
- Три файла, которые тестируют САМИ ML-runner'ы (`test_pyannote_runner`, `test_whisper_runner`,
  `test_pyannote_ref_isolation`), получили module-level `pytest.importorskip("torch")` —
  в облаке не собираются, а не падают.
- `pyproject.toml`: extra `cloud` (~40МБ, без torch/pyannote/faster-whisper/librosa/soundfile)
  — `pip install -e ".[cloud]"`.
- **Результат измерен на обеих конфигурациях:** без ML — `1415 passed, 7 skipped, 0 failed`;
  с полным стеком — `1459 passed, 3 skipped`, ровно как до правок (регресса нет).
- **Честное ограничение, записано в pyproject и в routine-док:** S0-регресс утечки
  reference-эмбеддинга в облаке покрыт лишь частично — сторож в `_diarize_batch` проверяется,
  сам runner нет. Полное покрытие только там, где установлен ML-стек.

### Changed — CONSTITUTION.md приведена к действующим решениям CP-0 (2026-08-08)
- Распоряжение владельца 2026-08-08: принять изменения, чтобы дальнейшее исполнение шло
  автономно и не упиралось в расхождение документа с кодом.
- **Ст. 16 (ревизия архитектуры) НЕ задействована.** Ни один принцип не изменён — исправлены
  только фактические ярлыки, расходившиеся с кодом и с собственными решениями владельца.
  Это требование Ст. 19.1 (непрерывность истины); прецедент — реконсиляция 2026-05-29.
- ASR (C-01): `faster-whisper large-v3` → `GigaAM-v3-RNNT` основной, Whisper остаётся
  поддержанным fallback'ом (`asr_backend: whisper`) — строки 55, 92, 178, критерий отката 374.
- Диаризация (C-03): «pyannote 3.3.2» → runtime `pyannote-audio` 4.0.4 + модель
  `pyannote/speaker-diarization-3.1`; добавлен порог DER > 15% на своём корпусе как условие
  пересмотра (условие самого владельца от 2026-08-07).
- Блок обязательных хаков: строка про `use_auth_token` **исправлена, а не удалена** — fallback
  в `_load_pretrained` существует именно из-за расхождения версий, удаление потеряло бы знание,
  однажды стоившее отладки (`bugs.md` 2026-06-04). Добавлен второй невыводимый факт: pyannote
  4.x принимает аудио только в памяти, иначе torchcodec на Windows роняет диаризацию и роли
  молча становятся UNKNOWN при исправном ASR.
- VRAM-примеры (2.4, 9.2, 9.3): пара «Whisper ~3GB + pyannote ~1.5GB» → «GigaAM + pyannote ≈ 5GB».
  Сам инвариант (одна модель за раз, выгрузка перед LLM ~10GB) не тронут.
- Карточка (C-02): «≤500 символов» → «≤512 байт в UTF-8» с объяснением, почему байты:
  файл читает MacroDroid на Android, символьный лимит непроверяем на стороне потребителя.
- Не менялось: запрет облака, локальность, изоляция `user_id`, GPU-дисциплина, запреты
  ORM/Docker/Redis/WhisperX, «ошибки не проглатываются». Запись об отвергнутом WhisperX word
  alignment (строка 78) оставлена — она не устарела.

### Fixed — T-18: request-scoped дашборд, loopback, CSRF (P-TEN-05/P-WEB-01/02/03) (2026-08-08)
- **P-TEN-05 (S0):** `/api/users/select` мутировал модульную глобаль `_USER_ID` на весь процесс —
  две вкладки с разными профилями смешивали чтения и записи. Профиль теперь резолвится
  на КАЖДЫЙ запрос из cookie `cp_profile` через зависимость `_current_user`; ~30 эндпоинтов
  получают его как параметр. `_USER_ID` остаётся только стартовым дефолтом и не мутируется
  обработчиками (закреплено AST-тестом на отсутствие `global _USER_ID`).
- **P-WEB-01 (S0):** привязка к не-loopback адресу — ошибка старта (`assert_loopback_host`,
  IPv4/IPv6/`localhost`), без env-обхода. Мутации защищены Origin-проверкой и double-submit
  CSRF-токеном даже на loopback.
- **P-WEB-02 (S1):** `/api/tools/import-audio` читал тело целиком в память до проверки размера
  при лимите 512MB. Теперь потоковое чтение с прерыванием по превышению; существующие проверки
  (whitelist расширений, path traversal, Windows reserved names) сохранены.
- **P-WEB-03 (S2):** `on_event("startup")` → lifespan с сохранением и отменой задачи поллера;
  SSE-подписчики разделены по профилю; соединение читателя закрывается каждый тик (текло).
- **Правка оркестратора поверх агента — дыра в самой сути задачи:** `_current_user` возвращал
  значение cookie **без какой-либо валидации**. Cookie приходит от клиента, то есть браузер
  (или любая страница, дотянувшаяся до loopback) назначал бы себе tenant-идентичность
  произвольной строкой — ровно то, что T-18 обязан устранить. Добавлена валидация:
  форма слага (`identity.validate_user_id`) + существование профиля; неизвестный откатывается
  на стартовый дефолт с логом, `/api/users/select` на несуществующий профиль отвечает 404.
  Источник истины — таблица `users` через новый `DashboardDBReader.user_exists()`, а **не**
  `get_user_ids()`: та выводит профили из `calls` GROUP BY, поэтому только что заведённый
  пользователь без единого звонка получил бы отказ (поймано тестом с профилем `bob`).
  Регресс: `test_forged_profile_cookie_is_rejected`.

### Fixed — T-05: версионированные миграции, owner-триггеры, починка FTS (P-DB-01/03/06) (2026-08-08)
- **P-DB-01 (S0):** `_migrate` содержал `except Exception: pass` в трёх местах — ошибка миграции
  проглатывалась, приложение работало поверх неизвестной схемы. Введён `db/migrations.py`:
  9 пронумерованных миграций, журнал `schema_migrations` с контрольной суммой каждой,
  `PRAGMA user_version`, каждая миграция — своя транзакция с откатом и re-raise. Изменение
  контрольной суммы уже применённой миграции → `MigrationChecksumError` до любых действий.
- **P-DB-06 (S1):** `transcripts_fts` объявлял колонку `user_id`, которой нет в content-таблице
  `transcripts` → штатный `rebuild` внешнего индекса был невозможен. `user_id` убран из FTS
  (и в `schema.sql`, и миграцией для существующих БД); проверено, что ни один читатель не брал
  его из индекса — `search_transcripts`, `db_reader.search_calls` и `ask.py` фильтруют владельца
  через JOIN к `calls`. Тест: поиск даёт идентичные результаты до и после `rebuild`.
- **P-DB-03 (S1):** межвладельческие ссылки теперь отвергает САМА БД — `BEFORE INSERT/UPDATE`
  триггеры с `RAISE(ABORT)` на `calls`/`promises`/`events`. Composite FK не использован
  осознанно: SQLite не умеет `ALTER TABLE ADD CONSTRAINT`, это означало бы полную пересборку
  центральных горячих таблиц ради той же гарантии.
- `integrity_report()` (quick_check + foreign_key_check + поиск рассогласованных владельцев)
  для pre/postflight; найденные рассогласования только сообщаются, владелец не угадывается.
- **Правка оркестратора поверх агента — гейт был мёртвым кодом:** бэкап перед миграцией
  требовался только если вызывающий передал `backup_dir`, а **ни один боевой вызов его не
  передавал** (включая `cli/utils.load_config_and_repo` — общую точку входа почти всех CLI-
  команд). Боевая БД мигрировала бы без снимка, то есть порядок T-20 → T-05 не давал ничего.
  Гейт переделан на самовзводящийся: `data_at_risk()` = есть непримененные миграции И в БД есть
  хоть один звонок; каталог бэкапов выводится сам. Пустые/`:memory:`/свежие БД не задеты, тесты
  не сломаны. Регресс: `test_backup_gate_arms_itself_on_real_data`.

### Changed — T-13: жизненный цикл LLM-клиента и scoped cache (P-LLM-01/06) (2026-08-08)
- **Конструктор больше не ходит в сеть.** `LLMClient.__init__` вызывал `_verify_connection()` —
  реальный POST-completion на каждый созданный объект (латентность, нагрузка, трата токенов,
  невозможность создать клиент без живого сервера). Проверка вынесена в `check_live()`
  (`/health`), `check_ready()` (`/v1/models`, fallback на 1-токенный completion для старых
  сборок) и `ensure_ready()`. Последняя поднимает тот же `ConnectionError`, что раньше бросал
  конструктор, поэтому контракт «exit 2 при мёртвом сервере» у `ask`/`bulk`/`deep-extract`/
  `biography` сохранён — все вызывающие переведены явно. Пробы в кэш не пишут.
- **Идентичность кэша:** в ключ добавлены `user_id` (изоляция профилей — раньше докстринг прямо
  фиксировал «в ключ кэша НЕ входит») и `model_fingerprint`. Отпечаток заполняется, только если
  вызывающий заранее позвал `check_ready()`; иначе пустой — ограничение зафиксировано явно,
  фиктивный отпечаток не выдумывается. Старые строки `llm_calls` не удаляются, просто перестают
  попадаться.
- `ask_log.prompt_hash`: глобальный `UNIQUE` заменён на `UNIQUE(user_id, prompt_hash)` —
  второй профиль с тем же вопросом получал конфликт вставки и оставался без своей строки.
  Миграция идемпотентная, с сохранением данных.
- `LLMDecodeError` вместо тихого `text=None` при невалидном ответе транспорта; ретрай 5xx
  с тем же backoff, что и таймауты; `generate()` по-прежнему возвращает `None` (совместимость
  с biography/graph).
- **Правка оркестратора поверх агента (требование было в задании, агент его не выполнил и не
  упомянул):** усечённый ответ (`finish_reason='length'`) продолжал кэшироваться. Это неполный
  JSON, который repair-парсер достраивает догадкой — закэшировав его, мы фиксируем догадку
  навсегда: все последующие прогоны возвращают тот же обрубок, ни разу не сходив к серверу.
  Ровно «stale/incomplete как успех» из P-LLM-06. Добавлен гард `not final.truncated`.
  **Существующий тест `test_truncated_response_cached_as_is` закреплял этот дефект** — заменён
  на `test_truncated_response_absent_from_cache` + контрольный тест, что полный ответ
  по-прежнему кэшируется (гард не переусердствовал).
- Карта `.claude/rules/llm.md` приведена в соответствие (документировала старое поведение).

### Added — T-20: проверенный backup, restore и retention (P-OPS-01, S0) (2026-08-08)
- Backup/restore в проекте не было вообще: одна повреждённая SQLite-БД = потеря всей системы.
  **T-20 — гейт перед T-05:** версионированные миграции нельзя запускать на боевых данных
  без проверенного восстановления.
- `ops/backup.py`: снимок ТОЛЬКО через SQLite online backup API (`Connection.backup`) —
  сырое копирование живого файла отвергнуто, при WAL снимок может быть несогласованным.
- **Верификация — обязательная часть создания, а не отдельная команда:** `quick_check`,
  `foreign_key_check`, счётчики ключевых таблиц, SHA-256. Снимок сначала пишется во временный
  файл, проверяется и только потом публикуется `os.replace`; провал → временный файл удаляется,
  публикации и ротации НЕ происходит, последний хороший снимок остаётся.
- Манифест рядом со снимком: SHA-256, размер, `user_version`, версии приложения/Python/ОС,
  счётчики `calls`/`analyses`/`transcripts`/`contacts`/`events`.
- Preflight: свободное место (+20% и минимум 10MB) и запрет назначения внутри каталога-источника.
- `restore` отказывается перезаписывать существующий файл без явного флага; при перезаписи
  умеет предварительно снять снимок текущего состояния.
- CLI: `backup`, `verify-backup`, `restore --to` через ленивый dispatch (T-01) — ML не тянется.
- **Правка оркестратора поверх агента:** манифест писался обычным `write_text`, тогда как сам
  снимок публикуется атомарно. Краш в этот момент оставил бы усечённый манифест: `verify_backup`
  такой снимок отвергнет, а retention, работающий «по манифесту», сочтёт файл чужим и не удалит
  — накопился бы неудаляемый орфан. Переведено на `artifacts.atomic_write_text` (реюз T-08),
  а не на вторую реализацию атомарной записи.
- Тесты: `tests/test_backup.py` (10) — снимок под активной записью, битый архив не ротирует
  хороший, подделка манифеста, restore в temp с чистым `quick_check`, отказ при назначении
  внутри источника, retention не трогает посторонние файлы.
- Не сделано намеренно: интеграция чека в `doctor.py` (файл правил другой агент) и планировщик.
...
- `ask.py`: `llm_available(llm_url, timeout=2.0)` — проба `/health` (паттерн M1/doctor.py,
  ...
  ALTER — таблица уже существовала с A2 как prompt_hash-кэш; F13 переиспользует колонку для
  метрик качества) + `_save_cache` теперь пишет `answered=1 если citations иначе 0`.
- **Бот:** `handle_plain_text` (F2) расширен — если НЕТ pending-напоминания (или оно истекло),
  свободный текст падает в `_handle_ask` вместо молчаливого игнора. Cap вопроса 500 символов
  (обрезка, не отказ). LLM доступен → `ask.answer_question` реюз ЦЕЛИКОМ (инъекция-гард A2
  не тронут) + HTML-форматирование ответа (`<b>`/`<i>` для источников, весь текст экранирован
  `html.escape` до вставки в теги — защита от HTML-инъекции через LLM-ответ или транскрипт-цитату
  в Telegram `parse_mode=HTML`) ≤4096 символов (ответ обрезается ДО экранирования на 3500,
  чтобы не резать финальную HTML-строку посреди тега). LLM недоступен → `ask.retrieve(k=5)`
  (OR-поиск по токенам — точнее для NL-вопросов, чем phrase-match `cmd_search`; автономное
  отклонение от буквы спеки "реюз cmd_search:312" в пользу уже существующего retrieve() —
  он и есть "top-5 сниппетов FTS", дублировать вторую FTS-реализацию не было смысла) с шапкой
  «LLM спит — прямые цитаты по запросу:». Не-allowlisted chat_id — игнор до любой ветки
  (`_get_user_id` гейт уже стоял с F2). Исключение из `answer_question`/`retrieve` — лог,
  пользователю дружелюбный текст без traceback.
- **Автономная интерпретация:** F2 уже предложил напоминание для ЛЮБОГО confirmed item_kind
  (event/promise), не только promise/deep_fact как в тексте спеки — /promises сегодня выдаёт
  ТОЛЬКО kind=event, буквальное ограничение спеки сделало бы фичу нерабочей для её же
  примера из цели задачи ("в пятницу ты обещал Ивану документы").
- Tests: `tests/test_ask.py` +5 (answered=1/0, llm_available true/ConnectionError/non-200) +
  `tests/test_bot_ask.py` (6: ответ форматирован+≤4096+источники, FTS-деградация без LLM-вызова,
  не-allowlisted игнор, ask_log пишется, cap 500 символов, исключение не течёт в чат) + 2
  обновлённых в `test_reminders_bot.py` (no-pending/expired-pending теперь падают в ask-путь,
  не «ничего» — переименованы под новое корректное поведение). Security-review НЕ запускался
  (T1, внешний ввод идёт через уже прорецензированный A2 инъекция-гард, новых SQL write-path
  нет — только аддитивная колонка существующей таблицы). 1079 passed/2 skipped.

## [Unreleased]

### Added — интерактивная схема архитектуры v5 (2026-08-07)
- `ARCHITECTURE_SCHEMA.html` полностью переписан: статичная устаревшая схема (D:\calls,
  «8 проходов») заменена на интерактивную SVG-диаграмму по `ARCHITECTURE_v5.md`.
- 6 слоёв (Вход → Обработка аудио/GPU → LLM-анализ → SQLite → Интеллект → Доставка/UI),
  21 узел с тултипами, 24 ребра, 7 потоков данных с панелью выбора справа: полный pipeline,
  bulk-load/enrich, граф знаний (BS-index), возраст (Ф0–Ф3), биография (11 фаз), дашборд/SSE,
  CLI. Выбор потока подсвечивает весь маршрут (узлы+рёбра), клик по узлу — описание и его потоки.
- Самодостаточный файл (инлайн CSS/JS, без зависимостей), адаптивен, тёмная тема.
- Валидация: встроенный JS прошёл `node --check`.
...
- Аудит marker+age_style нашёл 9 препятствий (P1-P9): лексиконы-омонимы («аккуратно»/«диалог»/
  ...
  watcher, ложно-узкий интервал ±1 год, pension/ЕГЭ без контекст-гардов, обвал conf при конфликте
  низшего класса, кнопка только при существующей строке стиля и только style-пасс,
  owner_birth_year=0 без диагностики. План исправлений (6 фаз, для Sonnet) — `fixager.md`.
...
- **Симптом (найден по запросу юзера — "не будет ли мешать" LLM-промпту age_v001.txt):**
  ...
  marker/relation-оценки, БЕЗ проверки, что она согласна со стилем. Если явный маркер
  говорил, скажем, G2, а стиль уверенно давал G6, код мог выдать УВЕРЕННЫЙ, но неверный
  ответ (бонус применялся вслепую) — прямое нарушение vozrast.md §7.1/§7.2 («Conflict —
  штраф за противоречия маркер vs стиль», «при конфликте маркер побеждает, стиль лишь
  слегка сдвигает интервал»). Реализован был только ПОЛОВИНА Conflict-члена (внутренняя
  бимодальность), не marker-vs-style половина.
- **Fix:** `scorer.score_contact(..., marker=None)` — явный маркер (birth_year_low/high
  + confidence из `contact_age_estimates`, только method IN marker/relation/combined —
  'llm' исключён как циклическая самопроверка того же рода слабого сигнала) входит в
  взвешенный пул как узкая сильная посылка (вес ~2.5×confidence/100 — обычно перевешивает
  СУММУ style-голосов за счёт концентрации на одной группе); детектируется конфликт
  argmax(style-only) != argmax(marker) → новый warning «расходится с явным маркером» +
  штраф в Conflict-члене формулы доверия. `confidence(has_marker: bool)` →
  `confidence(marker_strength: float)` — бонус пропорционален силе маркера, не плоский флаг.
  3-tuple возврат `score_contact` (добавлен `marker_conflict`).
- **Заодно (тот же аудит):** список «Личности» (`get_people`) показывал возраст ТОЛЬКО из
  marker/LLM-системы — контакт без явного маркера (для которых age_style и строился) не
  получал возраст в списке вообще, даже имея стилометрическую оценку. Добавлен guarded
  fallback на `contact_age_style` (маркер побеждает при наличии, иначе стиль), новое поле
  `age_source: 'marker'|'style'|None`, tooltip в app.js помечает «(по стилю речи)».
- 733 passed/2 skipped (полный `tests/`), +6 новых тестов (marker vote/conflict/fallback).
- Карта: `.claude/rules/insight.md` → «Возраст-стиль» дополнена секцией «Marker-vs-style».
...
- Новый презентационный слой `dashboard/labels_ru.py`: переводит весь видимый человеку
  ...
  promise_breaker→«нарушает обещания», severity, тип сущности, типы фактов, тренд, эмоц.паттерн)
  в RU. Источник английский намеренно (psychology_profiler кормит англоязычные книжные промпты —
  не трогаем); перевод — только на показе. `severity` КЛЮЧ сохранён (фронт красит по нему) +
  `severity_label`; англ. `label` паттерна русифицируется пофразно. Идемпотентно, неизвестное
  значение не теряется.
- `db_reader`: `get_entity_profile`/`get_character_profile` → `localize_character`,
  `get_person_dossier` → `localize_dossier`; `_build_character_{label,summary}` берут RU из карт.
- `app.js`: entity-модалка (`renderEntityTab`) — все статичные подписи RU (Метрики/Психопрофиль/
  Большая пятёрка/Паттерны…); досье/модалка показывают `severity_label`. Найдено: db_reader.py
  с Desktop пользователя функционально уже в main (67d313b) — регресса нет, добавлена русификация.
- Тесты: `tests/test_dashboard_labels_ru.py` (офлайн, чистый). 698 passed, 2 skipped.

### Added — «Возраст контакта» РЕАЛИЗОВАН: Ф0-Ф3 плана age-estimation (2026-06-11)
- **Ф0/Ф1 (детерминир.):** `insight/age_markers.py` (прямые маркеры: «мне 45 лет»/словесные
  числа/«1978 года рождения»/юбилей; этапные: пенсия/внуки/армия/сессия/ЕГЭ/школа; third-person
  guard) + реляционные якоря направленные («мам» владельца ≠ «пап» контакта; одноклассник ±2) —
  только при `owner_birth_year` в конфиге. `insight/age_estimate.py` — агрегатор в пространстве
  ГОДА РОЖДЕНИЯ (возраст не протухает; дашборд выводит к текущему году), классы точности,
  конфликт → высший побеждает + conf=min+10, согласие → +10 (cap 95).
- **Ф2 (LLM, Qwen3.5):** `configs/prompts/age_v001.txt` (один JSON, без markdown), парсер срезает
  `<think>`/fences/чинит обрыв; verbatim-гейт (галлюцинация −15, 0 валидных → отброс, но кэш);
  memoization sha1(prompt+age-v1) per-row; det-пересчёты переиспользуют оплаченный LLM-результат.
- **Ф3 (UI):** досье — секция «Возраст» (~точка (диапазон) · уверенность/100 + цитаты);
  список людей — колонка «Возраст» (серым при conf<50). `get_people`/`get_person_dossier` guarded.
- **Динамика:** маркер-часть в watcher-autofit инкрементально (`stale_only` — только контакты
  с новыми звонками); LLM в watcher запрещён (GPU). CLI: `age-estimate --user X [--contact N] [--llm]`.
- Таблица `contact_age_estimates` (+birth_year_*, llm_prompt_hash/llm_result — расширение DDL
  плана под динамику/кэш). Тесты: 691 passed (+33). security-review clean; ложные
  «CRITICAL/HIGH» code-ревьюера отклонены пересчётом (cap после llm-цикла, юбилей ±1 корректен).
...
- 2026-07-03 — fix+feat(age): fixager Ф1-Ф5 (лексиконы =-точный матч, support=хиты, z по полной популяции, честная ширина интервала, маркер-гарды, мягкий конфликт) + ансамбль v2 задача B: маркеры born_in/since_year/age_slangnum/age_approx/год-якорные события, KIN-арифметика (kin_child_age/stage, kin_parent_age, kin_grandchild), стиль-оси discourse/kancelyarit/morphosyntax/tempo + популяционный приор, биграммы лексиконов, реалии пере-датированы (reminiscence bump, пересечение эпох), TABLE/RULES v2. План: docs/superpowers/plans/2026-07-03-age-ensemble-v2.md. 792 passed.
- 2026-07-03 — feat(age): ансамбль v2 задача C — insight/age_fusion.py (fuse-v1, единая оценка на чтении: marker∩style/конфликт/style-only cap 70, llm cap 50), get_people+get_person_dossier читают fused, кнопка «Пересчитать возраст ↻» = маркер-пасс контакта + стилометрия популяции + hints (owner_birth_year, diarization). 810 passed/2 skipped. Память: insight.md v2-секции, CONTINUITY.
- 2026-07-05 — docs: ozalup2.md — единый план развития (ozalupennieStrategic5 + аудит meetily): новые M1-M8 (doctor, аудио-плеер с seek, llm_cache мемоизация analyze, json_mode+canary, drag&drop импорт, заметка владельца, error_message в UI, deep-extract map-reduce длинных звонков), инвариант инъекция-гарда, 12 отвергнутых идей; supersedes ozalupennieStrategic5 как входная точка.
- 2026-07-16 docs: OzaluplivanieFable2.md — Fable-ревизия 3 (Qwythos-9B отклонён §9, F28 единый возраст, F29-F32 психоглубина + петля качества); decisions.md + CONTINUITY обновлены; OzaluplivanieFable.md закоммичен (ревизия 2, действует по ссылке)
- 2026-07-17 — feat(capture): F4 голосовая заметка владельца → конвейер. filename_parser: формат `voicenote_{ts}[__{target}].ogg` → phone='self:notes', contact_name='Мои заметки', note_target_name; ingester создаёт/переиспользует спец-контакт заметок; repository.find_contact_by_name (exact→case-insensitive prefix→ambiguous); orchestrator: call_type='note' пропускает диаризацию (speaker=OWNER принудительно) и analyze, stage=4/status='done' сразу после транскрипта; _maybe_bind_note_to_contact — caption `@Имя` → contact_notes (M6, append, cap 2000) при однозначном совпадении; _finalize_note шлёт send_note_ready (первые 200 симв.); telegram_bot.handle_voice_note — allowlist chat_id, cap 50MB, атомарная запись `.part`→replace, caption→sanitized target в имени файла. Дашборд: фильтр «🎙 Заметки» (call_kind=note) в /api/calls + calls-таблице. Найден некритичный баг устойчивости (bugs.md #10: asyncio.get_event_loop хрупок к порядку asyncio.run в тестах) — не прод-баг, задокументирован. 1101 passed/2 skipped (+22). Память: pipeline.md (note-ветка), bugs.md.
- 2026-07-17 — feat(deliver): F5 вечерний отчёт дня (21:00) + случайное воспоминание. `deliver/daily_report.py::build_daily_report(conn,user_id,date)` — 5 опускаемых-если-пусто секций: 📞 Сегодня (N звонков/M мин/топ-3 по длительности), 📌 Новое (promise/debt дня из A1-реестра, reuse `digest._merged_open_items` — rejected скрыт F1, confirmed помечен ✓), ⏰ Завтра (F2-напоминания на завтра + просроченные открытые через `digest.overdue_items`), ⚠️ Ошибки (M7 error_message ≤100 симв.), 🎲 Воспоминание (случайная events.source_quote старше 180 дней, `ORDER BY RANDOM()`); итог клипается ≤4096 символов. Новый `deliver/telegram_sender.py::send_telegram_message` — голый sync HTTP POST sendMessage (НЕ TelegramNotifier: тот шлёт только когда self.app живой, т.е. изнутри running bot-процесса — watcher работает в отдельном процессе, self.app там всегда None). Триггер — `FileWatcher._maybe_send_daily_report()` раз в цикл run_loop: local hour≥21 И `report_state.last_report_date≠today` (новая таблица, insight/repository.py) → build → send → state двигается ТОЛЬКО при успехе (сбой build/send — лог, ретрай след. циклом, цикл не падает). CLI `daily-report --user X [--date] [--send]`. 12 тестов (секции/капы/report_state/watcher-триггер: skip до 21:00, once-per-day, no-chat_id skip, send-failure не двигает state, build-failure не роняет цикл). 1113 passed/2 skipped (+12). Память: pipeline.md (шаг 8 watcher-цикла).
- 2026-07-17 — feat(ops): F6 watcher heartbeat + плановый doctor-отчёт → Telegram 🟢/🔴. `watcher._write_heartbeat()` — touch `data_dir/watcher.heartbeat` каждый цикл (run_loop + run_once). `doctor.py` +6 чеков: heartbeat (нет файла→WARN, mtime>3×watch_interval_sec→FAIL), queue-stuck (не-терминальный >6ч→FAIL, call_id-список≤5), error-burst (>20% и ≥3 звонков в error за 24ч→WARN), disk (свободно <5GB→FAIL), reminders-stale (guarded — просрочено>24ч и enabled→WARN «бот не тикает»), input-silence (и файл, и звонок старше 72ч→WARN, WireGuard/FolderSync-подсказка §8). Новый `doctor.build_doctor_message` — заголовок 🟢 Осмотр пройден / 🔴 Есть проблемы (N FAIL) + полный отчёт. `cmd_doctor --send` — рассылка всем users с telegram_chat_id (reuse F5 telegram_sender). Плановый прогон — `watcher._maybe_send_doctor_report()`: local hour≥9, независимый дедуп-столбец `report_state.last_doctor_date` (та же таблица, что F5 — оба пуша могут сработать в один цикл, не дублируются день-в-день). новые тесты в test_doctor.py (6 чеков × позитив/негатив, build_doctor_message, cmd_doctor --send, watcher heartbeat-write + doctor once-per-day + skip до 9:00). 1131 passed/2 skipped (+18). Память: pipeline.md (шаг 8 расширен heartbeat+doctor).
- 2026-07-17 — feat(dashboard): F7 панель «Здоровье системы» — тот же doctor-отчёт (F6), что уходит в Telegram, виден в дашборде без бота. `GET /api/health-report` — `doctor.run_checks(config,conn)` напрямую (side-effect-free, никакой записи в БД) в threadpool (`loop.run_in_executor`, llm-чек делает сетевой запрос с timeout=2s — не блокировать event loop); conn берётся через существующий `_get_reader()` seam (тестовый mock-инжект `_DB_READER` работает без изменений). UI: collapsible-панель `#health-panel` на overview (строка на чек, иконка+цвет по статусу OK/WARN/FAIL/SKIP, кнопка «Обновить»), 🔴-бейдж `#header-health-badge` в шапке при любом FAIL — обновляются в `loadHealth()`, вызываемой из `loadOverview()` (на initial-load и при переключении на вкладку). 4 теста (структура ответа, FAIL отражён, `_CONFIG=None`→пустой список, run_checks вызван без побочных SQL-эффектов). 1135 passed/2 skipped (+4). Визуальная проверка UI — на боксе (dev-машина без реальной БД). Память: dashboard.md (+секция overview).
- 2026-07-17 — feat(insight): F8 Эббингауз-тиры контактов (core/active/warm/cold/archive) — «память, которая забывает». `insight/tiers.py`: `score = retention(days_since_last_call, strength(call_count)) · log1p(total_talk_minutes)`, `retention = exp(-days/(30·strength))`, `strength = 1+log1p(call_count)` (касание поднимает, тишина опускает); тир по ПЕРЦЕНТИЛЯМ score внутри юзера (top 5%→core, до 25%→active, до 60%→warm, до 90%→cold, хвост/0 звонков→archive) — калибровка бесплатно, без абсолютных порогов. `contact_tiers` UPSERT, старый tier переносится в `prev_tier` (переходы возвращаются для будущих C3/F5 «перешли в остывшие»). Триггеры (все дёшево, non-fatal): watcher `_run_insight_fit` — РЕАЛЬНЫЙ автоматический ночной хук (debounced autofit каждый цикл `watch`, а не только ручной `bulk-enrich`, которым код изначально ограничивался до ревью) · конец `bulk_enrich()` · после `obligations-digest`. CLI `tiers-recompute --user X`. Потребители: `bulk/enricher.py::select_pending_calls` (новая, вынесена из `bulk_enrich`) — очередь `ORDER BY` тир (core первым, `TIER_RANK_SQL_CASE`), деградирует к прежней хронологии если `contact_tiers` ещё не существует; дашборд `get_people`/`get_person_dossier` — колонка/бейдж с русской меткой (`labels_ru.TIER`) + сортировка списка людей по тиру первично. Biography per-entity очередь (p5_portraits и т.п.) сознательно НЕ тронута — план явно резервирует это для будущей F21 («ночной хук после signals-recompute; очередь по тирам F8», уже резолвит entity→contact через `entity_contact_map`); реордер внутри exhaustive checkpoint-based биографических проходов имел бы низкую ценность и лишний риск для чужой задачи. 15 новых тестов (формула монотонна/касание поднимает/архив при 0 звонков, перцентильные границы на 20 контактах, prev_tier фиксирует переход, user_id-изоляция, schema идемпотентна, enricher-очередь отсортирована/деградирует без таблицы, dashboard get_people/get_person_dossier отдают tier+tier_label, watcher-автофит вызывает recompute_tiers). 1150 passed/2 skipped (+15). Память: insight.md (новая секция «Тиры контактов» + Таблицы/Файлы), CONTINUITY.
- 2026-07-17 — feat(insight): M8 Deep-extract — map-reduce по ПОЛНОМУ транскрипту длинных звонков (снимает слепоту head+tail-клипа основного analyze-промпта, llm.md: >3000 симв → 1500+1500). `insight/deep_extract.py`: `chunk_text()` — символьные чанки 9000/800 overlap, разрез строго по word-boundary (короче size → 1 чанк; overlap>size → step=max(1,size-overlap), не виснет); каждый чанк — отдельный LLM-вызов через `LLMClient(cache_conn=conn, prompt_version="deep-v1")` (json_mode, temperature 0.1, max_tokens 900) — переиспользует memoization M3 (`llm_calls`), НЕ отдельный кэш; промпт `configs/prompts/deep_extract_v001.txt` с инъекция-гардом (инвариант 12) в тегах `<фрагмент>…</фрагмент>`. Схема (`insight/repository.py`): `deep_facts` (PK `user_id,item_key` — sha1(call_id|type|what[:60])[:16], дедупит перекрытия соседних чанков через INSERT OR IGNORE) + `deep_scans` (гейт повторного прогона без `--force`, PK `user_id,call_id,prompt_version`). Границы (НЕ нарушены): результат — своя таблица, НЕ events/graph (replay-инвариант graph.md, прецедент B2) — чистый дисплей-слой. Гейты на item: `who∉{OWNER,OTHER}` → дроп, `quote` не substring чанка → дроп ЦЕЛИКОМ (факту без цитаты веры нет), `what` пустой → дроп, `type∉{promise,debt,fact,date}` → дроп. Отбор звонков: `status IN (done,transcribed)`, `duration_sec>=min_duration`, `call_type IS NULL` (F4 voicenote-заметки исключены — не отношение с контактом, та же логика что F8 `select_pending_calls`); `min_priority` задан → INNER JOIN analyses (transcribed-без-анализа не проходит). CLI `deep-extract --user X [--min-duration 600] [--min-priority N] [--limit 100] [--force]`; llama-server недоступен → exit 2 (паттерн `ask.py`/`bulk.py` — `LLMClient.__init__` поднимает `ConnectionError`), пустая выборка сеть вообще не трогает (клиент строится лениво на первом реальном чанке). Потребители: `deliver/digest.py::build_digest` получил `extra_sections` параметр ([(заголовок,[строки])], пустые секции пропускаются) — `obligations-digest` CLI подключает `recent_deep_lines` как секцию «🔎 Из глубокого прохода» (только promise/debt, ≤300 симв/строка); dashboard `get_person_dossier` — секция «Из длинных разговоров» (guarded `_has_table`, top-5 по created_at) + app.js рендер рядом с «Факты (дословные цитаты)». 22 новых теста (chunk_text: не рвёт слова/overlap виден/короткий→1 чанк/step-guard; item-гейты по одному на каждое условие; дедуп одинакового факта из 2+ чанков → 1 строка; повторный прогон без --force → 0 новых LLM-вызовов; --force пересчитывает; min_duration/min_priority/call_type='note' фильтры; user_id-изоляция; пустая выборка не трогает LLMClient; recent_deep_lines guarded+truncated; build_digest extra_sections рендерится/пропускается пустым; досье guarded без таблицы + наполнено top-5). 1172 passed/2 skipped (+22). Память: insight.md (новая секция «Deep-extract» + Файлы), CONTINUITY.
- 2026-07-17 — fix(insight)+feat(insight): §4.1 нормализованный quote-гейт (шум-доктрина) + F26 осторожный deep-extract голосовых заметок. Ретроактивный фикс M8 (пропущенная поправка Fable §4.1 — «первая задача, которой нужен `norm_quote`, создаёт его»): новый `src/callprofiler/textnorm.py::norm_quote()` (lower→ё→е→убрать пунктуацию→схлопнуть пробелы), M8-гейт `deep_extract._save_item` переведён с сырого `quote in chunk` на `norm_quote(quote) in norm_quote(chunk)` — сырой quote по-прежнему хранится/показывается дословно, нормализация только для проверки присутствия (ASR-вариации пунктуации/регистра/ё больше не режут валидные находки). F26: `call_type='note'` (F4) теперь ВХОДЯТ в deep-extract по умолчанию со своим `NOTE_MIN_DURATION=30` (не зависит от `min_duration` обычных звонков) — реверс M8-исключения. Ветка для заметок ЖЁСТЧЕ: `who` фиксирован ТОЛЬКО `OWNER` (LLM предложила `OTHER` на моно-голосовой заметке → галлюцинация, дроп); `type` ⊂ {promise,fact} (нет debt/date); новый **числовой гейт** — `extract_numbers()` (цифры + словарь словесных числительных «один..двадцать, тридцать..девяносто, сто..тысяча», БЕЗ композиции «двадцать три»→23, консервативно) — цифры в `what` обязаны присутствовать в `quote`, иначе дроп (искажение сумм — юзер-риск №1, юзер 2026-07-07). `contact_id` заметки всегда из БД (спец-контакт «Мои заметки», НИКОГДА из LLM-вывода). Потребители: `recent_deep_lines` префиксует note-строки 🎙; `get_person_dossier` подавляет секцию «Из длинных разговоров» для контакта `phone_e164='self:notes'` (self-леджер живёт в digest/бота, не в чужом досье). 19 новых тестов (`test_textnorm.py` ×6: lower/ё/пунктуация/пробелы/none-safe/ASR-вариация; deep-extract F26 ×13: extract_numbers чистые ×3, OWNER-фиксация+дроп OTHER, type-фильтр debt/date, числовой гейт совпадение цифрами/словом/отсутствие чисел, NOTE_MIN_DURATION независим от min_duration звонков, contact_id из БД не из LLM, повторный прогон бесплатен, 🎙-маркер в digest, досье-подавление для self:notes). Удалён 1 устаревший M8-тест (`test_note_call_type_excluded` — проверял поведение, которое F26 сознательно отменил). 1190 passed/2 skipped (+19/-1). Память: insight.md (секция Deep-extract переписана под F26 + §4.1), CONTINUITY.
- 2026-07-17 — feat(insight): B1 темп и ритм речи из таймстампов (oz5). Роутер: `feature_store.build_contact_features` SELECT сегментов расширен до `call_id, speaker, text, start_ms, end_ms` (было только speaker/text) — существующие текст-фичи читают из dict только свои ключи, обратная совместимость подтверждена полным регрессом tests/insight (352 passed). Новый `insight/features/tempo.py::compute_tempo(segments, reference_now=None)`, зарегистрирован в `_TEXT_FNS`: `tempo_cps` — знаков/сек речи КОНТАКТА (только OTHER-сегменты с dur≥500ms, короткие — шум ASR-нарезки, отбрасываются; support_n=число сегментов); `reply_latency_ms` — медиана пауз по смежным парам OWNER→OTHER ОДНОГО call_id (0<gap<15000ms — за пределами это перебивание или разрыв записи, не пауза на обдумывание; support_n=число пар); `tempo_accel` — медиана по звонкам отношения cps(последняя треть OTHER-сегментов)/cps(первая треть) — звонки с <6 OTHER-сегментов пропускаются (support_n=число звонков). Нет end_ms/пусто → ключ просто не эмитится (роутер терпит отсутствие). Лейблы в `labels.py` (тройки фраза-high/фраза-low): «темп»/«паузы»/«разгон». Досье отдельной секции не получает — оси автоматически попадают в существующие черты-фразы (distinctive_dims). 13 новых тестов (cps точное значение ±0.01, короткие сегменты отброшены, OWNER/UNKNOWN исключены, нет таймстампов → пусто, латентность-медиана, UNKNOWN рвёт смежность пары, gap вне диапазона отброшен, кросс-звонковая пара игнорируется, tempo_accel ratio на 9 сегментах/3-третях, <6 сегментов пропущено, медиана по нескольким звонкам, когортный тест «быстрые vs медленные» — mean diff > 1σ). 1203 passed/2 skipped (+13). Память: insight.md (ROBUST-строка таблицы «Фичи» +tempo.py), CONTINUITY.
- 2026-07-17 — feat(insight): B2 специфичность vs вода (oz5). Новый `insight/features/specificity.py::compute_specificity(segments)` — доля токенов речи КОНТАКТА, несущих числа/даты/деньги/время; ключ `specificity`, tier ROBUST, support_n=число хитов. Считает на whitespace-токенах (`text.split()`), НЕ на `base.tokenize()` — тот вырезает цифры целиком регексом `[а-яёa-z]+`, а числа здесь главный сигнал. Категории (numeric `\d`, month/weekday prefix-матч по normalize_lemma, money `руб|₽|тыс|млн|долла|евро`, time `\d{1,2}:\d{2}`) НЕ эксклюзивны — один токен («15:30») даёт и numeric-, и time-хит одновременно, усиливая сигнал. Зарегистрирован в `_TEXT_FNS`. Лейбл в `labels.py`: «конкретность» (высокое — «говорит конкретно: числа, даты, суммы», низкое — «говорит обтекаемо, без конкретики»). **BS-index v2 НЕ делается** (подтверждение решения T3-ревью 2026-07-02, decisions.md) — specificity display-level, в граф не льётся (replay-детерминизм graph.md). **Отступление зафиксировано в decisions.md** (новая запись «B2 specificity»): канонизированные graph-entity хиты НЕ считаются в v1 — потребовали бы протащить alias-set через сигнатуру роутера, YAGNI без измеренного разрыва. 10 новых тестов (конкретное предложение → высокое значение точным ±0.01, водянистое → 0.0, month/weekday prefix-хиты, один токен даёт 2 независимых хита, money-варианты, OWNER-речь не считается, UNKNOWN fallback не исключается, пустые сегменты/текст → {}, когортный тест конкретные vs водянистые — mean diff > 1σ). 1213 passed/2 skipped (+10). Память: insight.md (ROBUST-строка +specificity.py), decisions.md (новая запись), CONTINUITY.
- 2026-07-25 — docs: `opsus5.md` — сводный план исправления и развития (новая входная точка исполнения, supersedes OzaluplivanieFable2/OzaluplivanieFable/ozalup2/NeErrorsGR как рабочий список). 36 атомарных самодостаточных задач с точными якорями, сверенными по дереву на 2026-07-25. Часть I (13 дефектов): ключи леджера payload/what (summary_builder:421-459 пишет оба, db_reader `_norm_ledger_item` — единственная точка нормализации, app.js fallback) · `deliver/notify.py::run_coro` вместо двух расходящихся asyncio-путей (orchestrator:558/978) · единый источник risk-порогов `risk_calibration.risk_bands` для карточки/summary/дашборда (убирает BSCalibrator на risk-шкале и литералы 60/30 vs 70/40 vs калиброванные) · логирование сбоя unload (orchestrator:621/625, OOM-профилактика) · doctor/admin/data_extractor под user_id · удаление мёртвого event_bus (asyncio.run травил loop потока) · GET /v1/models вместо тестового completion · thread-local коннект + busy_timeout · запрет не-loopback bind дашборда + аудит мутаций · приоритетное усечение карточки до 512 байт. Часть II: F28 единый возраст (fuse_age — единственное видимое число, marker/style → details, AGE_DISPLAY_MIN_CONF=30). Часть III: /who, ночной бэкап+integrity, аудит заземления биографии, ворота достаточности материала. Часть IV: реестр `psy_signals` + производители (лингвистика, turn-taking, ритм, циркумплекс, адаптеры B-серии, взаимность, BS v2, стресс-контраст, зеркальная динамика, лонгитюд). Часть V: split-half стабильность (один проход ПОСЛЕ всех производителей), секционный портрет с критик-пассом и петлёй ✓/✗ (без промежуточной v1), детектор перемен, именование неизвестных. Часть VI: vault-экспорт, метрики, финализация + бокс-очередь команд. Собственные решения сверх исходных планов зафиксированы в CONTINUITY.- 2026-08-21 — merged PR #18 (superset of #17: WAL-sidecar restore fix + [full] extra + tools.py traversal), closed PR #17; CONTINUITY: code-level audit of sintezdiharea status — 11/26 confirmed, gaps T-18 (reprocess without user_id), T-08 (card_generator non-atomic write); next = T-07.
- 2026-08-21 — ci: fix apt line (ffprobe not a package, inside ffmpeg) + install package via pip install -e ".[cloud]" before pytest (workflow never installed the package); stale branch claude/adoring-mendel-b281nh deleted.
- 2026-08-21 — fix(biography): restore BUDGETS dict deleted in 5c0cb79 (NameError on every biography pass, 13 call sites); fix name_extractor logger→log; regression tests/test_biography_prompts_smoke.py. Surfaced by CI flake8 F821 once apt step was fixed.
- 2026-08-21 — docs: CLAUDE.md + AGENTS.md rewritten against current tree (sintezdiharea entry point, CP-0, migrations via ALL_MIGRATIONS, torch_patch, [dev,full]/[cloud] installs, ruff F821 gate, PR/parallel-session rule, revert protocol; T1 row fixed Opus+/fast, stale 90-tests/15-step/RISKS.md/serhio removed); db.md schema rule updated to T-05 migrations. Fact-checked by 4 verifier agents + self-grep.
- 2026-08-21 — docs(routines): bs-research-fable.md — промпт фундаментального исследования BS-index/понимания абонента для Fable+ultracode на боксе (research copy БД, loop-until-dry, adversarial verify, pre-registration, exploration/confirmation split, паралингвистика по originals, weak labels из promise_outcomes/fact_feedback; результат docs/research/bs/90-execution-plan.md).
- 2026-08-21 — docs(routines): bs-research-fable.md переписан под dev-ноутбук (без БД/GPU/LLM): консилиум 12 экспертов с протоколом спора (steelman→перекрёстный допрос→ledger C-n/Delphi, minority reports, вето статистика/этика), first-principles+anti-template гейты, refinement passes до идеала, box-package пре-зарегистрированных экспериментов для бокса.
- 2026-08-21 — docs(routines): bs-research-sol.md — тот же исследовательский протокол (консилиум/ledger/проходы/box-package) для OpenAI Sol Ultra на dev-ноутбуке: без Claude-механик и без упоминаний бюджета; остановка только по исчерпанию исследования.
- 2026-08-22 — docs(routines): bs-research-{fable,sol}.md §7 — контракт единственности/атомарности плана: 90-execution-plan.md = единственный deliverable, задачи R-nn (≤1 дня, обязательные поля, box-package как R-задачи с ветвлением, запрет зонтиков/исследований).
- 2026-08-22 — docs: bs-research-fable.md переписан как обычный промпт прямого выполнения (не routine): /goal с конечными целями, единственный deliverable 90-execution-plan.md с контрактом атомарности R-nn, новая обязательная часть — индекс уверенности 1–100 (калибровка, входы, уровень, отношение к admiralty/age-conf/T-15/T-23, UI при низкой уверенности), метод отдан модели (§5), требования — к свойствам результата.
- 2026-08-22 — docs: bs-research-sol.md переписан тем же образом (прямой промпт, Goal, единственный атомарный план R-nn, индекс уверенности 1–100, метод на усмотрение модели; без Claude-механик и бюджета, остановка только по исчерпанию).
- 2026-08-22 — docs(research): bs rounds 1-3 checkpoint — `docs/research/bs/`: реконструкция BS-пути по коду (4/5 членов v1_linear мертвы, who=UNKNOWN, verbatim только в replay), 151 источник, claims-ledger C-01..C-20 после 57 рецензий, judge panel, first principles (CR = Beta-binomial по promise_outcomes, CI = масса posterior ±0.1), 70-synthesis, 90-execution-plan R-01..R-30; box-package/hostile review — не начаты.
- 2026-08-22 — docs(research): bs round 3 — план `90-execution-plan.md` R-01..R-30 прошёл hostile review (stat/deception/arch/ethics; 19 правок, R-25/R-26 сняты, C-09 retracted), `70-synthesis.md`, `box-package/` (README/hypotheses/protocol/decision-rules/adjudication-request + scripts 00–08 с `--synth` smoke, 06 удалён); decisions.md абзац, bugs.md 🔴 D1–D3, CONTINUITY → следующий шаг = R-01 на боксе.
- 2026-08-22 — docs: BS-research отложен решением владельца (не применять; справка в docs/research/bs); CONTINUITY → Next = T-07 (+T-18); bugs.md D1–D3 Solution → оживить формулу (fact_type/who/verbatim), не заменять.
- 2026-08-22 — feat(T-07 slice): `calls.next_retry_at` (миграция 10) + exp-backoff с jitter в `update_call_status`, `get_error_calls` уважает `next_retry_at`; терминальный гард done/transcribed (`force=`); T-18: dashboard Reprocess / `retry_errors(user_id=)` user-scoped. Тесты: test_repository (+4), test_dashboard_tools, test_db_migrations. Оркестрация: Opus implement → Haiku test-gate → Sonnet review → ручная верификация диффа (2 отклонения от спеки исправлены лично).
- 2026-08-22 — docs(routines): bs-research-sol.md переписан под новый Goal — индекс пиздежа v2 ИЗ существующих сигналов, работает с первого звонка у нового пользователя, без калибровки на реальной БД/разметки; развивать v1_linear/admiralty/паттерны, не замораживать; deliverable docs/research/bs-v2/90-execution-plan.md; прежняя реконструкция/литература переиспользуются.
- 2026-08-22 — feat(T-06): purge_user по introspection (user_id → правило, иначе CHILD_RULES, иначе RuntimeError; FK off + foreign_key_check перед commit; тест полной схемы), `ops/purge_files.py` карантин файлов одним move в `data_dir/trash`, cleanup.py purge-user/keep-only → файлы; фикс `Config.load()` (не существовало) → `load_config(args.config, validate=False)`. Оркестрация Opus/Haiku/Sonnet; 3 отклонения агента исправлены лично (hand-order list, file-by-file move, несуществующий API).
- 2026-08-22 — feat(T-08 slice): карточки (card_generator, summary_builder) → atomic_write_text; архив оригинала (watcher restore, bulk-load) → atomic_copy_file; тест test_card_write_is_atomic_old_card_survives_crash. ingester/text_export уже были атомарны.
- 2026-08-22 — feat(T-12 slice): `_unload_models()` → bool + измеренный VRAM-барьер (`models.gpu_unload_barrier_mb`), `gpu_state`; при сбое выгрузки/барьера LLM-фаза пропускается (OOM-guard), `except: pass` убраны. Тесты test_orchestrator_roles (+2).
- 2026-08-22 — feat(T-11 slice): учёт упавших ASR-окон (`last_coverage`), `calls.asr_coverage` (миграция 11), гейт `models.asr_min_coverage` → error до stage 2 (оба пути), бейдж в детали звонка. Suite 1480 passed.
- 2026-08-22 — feat(T-14 slice): analyze_v002 (`{{owner_name}}` из users.display_name, envelope <данные>, правило «данные≠инструкции»), PromptBuilder: клип через clip_transcript_for_llm (models.prompt_max_chars), реальная длительность, нейтрализация закрывающего тега; PROMPT_VERSION_ANALYZE=v002 везде (кэш инвалидируется штатно). Тесты tests/test_prompt_builder.py (16). Агент: тест-файл записал в корень с битым путём, забыл 2 литерала v001 в orchestrator и local-import (F823) — исправлено лично.
...
- 2026-08-22 — feat(T-23): психометрика без инструмента (темперамент/Big Five/мотивация) удалена из профайлера, портретных промптов (p5-v4), досье, модалки, labels/models/config дашборда; тесты обновлены (220 passed в dashboard/biography-наборе). Workflow: gate/review-агенты упали с API 403 — гейт прогнан лично.
- 2026-08-22 — feat(T-15 slice): парсер не бросает на любом вводе; LLM недоступен/parse_failed/parsed_partial/output_truncated → error+backoff вместо выдуманного анализа (orchestrator + enricher); stub_short для коротких звонков; verbatim-гейт цитат в боевом графе (builder грузит транскрипт). Тесты: test_response_parser(+gates), test_analyze_gate, test_orchestrator_analysis_gate, test_graph_builder_transcript_gate.
- 2026-08-22 — test(T-24 slice): tests/test_tenant_matrix.py — матрица изоляции арендаторов по 14 read-методам Repository (чужие id невидимы, списки только свои); маркеры pytest `box`/`tenant`; docs/TESTING.md «Tiers».
- 2026-08-22 — feat(T-19 slice): RiskPolicy `risk_band`/`risk_emoji` (risk-v1) — карточка и дашборд красят риск по одним порогам (risk_thresholds, fallback 30/70); 8 литеральных сайтов в app.js и литералы db_reader заменены; tests/insight/test_risk_policy.py.
- 2026-08-22 — feat(T-09 slice): `Orchestrator._fail()` — единый владелец статуса error с классификацией FATAL/retryable; FATAL исключены из авто-повторов и видны в doctor dead-letters; bulk-load → `transcribed`+stage 2 ПОСЛЕ транскриптов (не `done` до). Тесты: test_loader_status, test_orchestrator_error_classification, test_repository_error_handling, test_doctor_dead_letters.
- 2026-08-22 — chore: серия sintezdiharea T-06…T-25 (срезы) завершена в режиме оркестрации; полный suite зелёный (см. CONTINUITY).
- 2026-08-22 — fix(T-19): `risk_band` границы приведены к прежнему контракту risk_emoji (🟢 <green_max, 🟡 ≥green_max, 🔴 ≥yellow_max; tests/insight/test_risk_calibration.py — авторитет); агентские тесты с противоречащими границами исправлены/удалены; флаки-тесты T-09 (tempfile на Windows) заменены fixture-based.
- 2026-08-22 — docs(T-26): `docs/100bsindex.md` — план прямого исполнения BS-v2 (observed discrepancy + evidence confidence) на основе `docs/GPT90-execution-plan.md`, сверенный с `main@11fe81e`: migration 12 (не 11), prompt-candidate v003 (v002 уже в проде), purge-совместимый trigger снапшотов, 11 якорных правок; R-01…R-52, старт Phase A R-01.
- 2026-08-22 — feat(T-26/R-01): миграция 12 `bs_v2` — canonical `contact_bs_metrics`, immutable `bs_legacy_snapshots` (purge-совместимый DELETE-триггер через `users.purge_started_at`), `relation_evidence` ledger, provenance-колонки promises/events, projection-колонки contact_summaries/entity_metrics/bs_thresholds/relations, placeholder-контакты для звонков без contact_id + baseline 0/1; три DDL-владельца сведены к одному тексту (`db/bs_schema.py`), sqlite_master больше не зависит от порядка init_db/apply_graph_schema/apply_insight_schema. Тесты: test_db_migrations.py::test_migration_12_bs_v2_full_contract (+rollback, +purge).
- 2026-08-22 — feat(T-26/R-02,R-03): identity-чтения графа требуют `user_id` (`get_entity`/`get_entity_metrics`/`full_recalc_from_events`/`get_behavioral_patterns`/`get_social_position`), инвентарный tenant-тест распространён на GraphRepository/EntityMetricsAggregator; replay селектит `calls.call_id` (колонки `calls.id` не существует — запрос падал, ошибка глоталась, replay молча обрабатывал 0 звонков).
- 2026-08-22 — feat(T-26/R-04,R-05): `analyze/payload_reader.py` (canonical_json → raw_response → invalid; builder обещал канон в комментарии, но не запрашивал колонку) и `analyze/transcript_format.py` — ОДИН role-tagged формат `[me]/[s2]/[?]` для live/bulk/builder (было три несовместимых, из-за чего FactValidator в боевом пути не определял говорящего). Тесты: test_bs_event_provenance.py.
- 2026-08-22 — feat(T-26/R-08): `analyze/roles.py::canonical_who/side_of` — одна нормализация роли (`Me|S2` промпта → `OWNER|OTHER`); `save_promises` сохраняет `vague`, заземляет цитату (`source_quote`/`quote_match`) по role-tagged транскрипту; regex-парсер больше не теряет `vague`; `_side` в promise_outcomes/digest понимает live-формы — live-обещания впервые попадают в outcomes.
- 2026-08-22 — feat(T-26/R-13,R-14): `insight/bs_index.py` — чистые `compute_bs_v2` (v2_roc_observed_1) и `compute_bs_confidence` (c1_effective_evidence_1) + `confidence_value`; согласие A и устойчивость S ВЫЧИСЛЯЮТСЯ из вкладов кластеров (первая реализация подставляла константы 0 — тесты проходили, ничего не доказывая); таблицы §5.4 проверены дословно (clean 1/9/18/52/65, A=1 1/18/34/77/97, raw-n B/L/M).
- 2026-08-22 — feat(T-26/R-15): `insight/bs_snapshot.py` — read-only снимок оснований контакта (as_of-горизонт, детерминированный порядок, каждый запрос по user_id); legacy-`contradiction` из bulk-пути в C-кандидаты не попадает (RISK-26).
