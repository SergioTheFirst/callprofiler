...
src/callprofiler/insight/
  ...
  features/{base,temporal,reciprocity,trajectory,linguistic,formality,pronouns}.py
  synth/{corpus,archetypes,noise,phrasebank}.py
  # feature_store.build_contact_features маршрутизирует META(calls)+TEXT(segments)
cli/commands/insight.py        tests/insight/*
```

## Чего ещё НЕТ (отдельные планы)

Фаза 3 affective/topical · Фаза 4 dominance (gated по UNKNOWN) · Фаза 5 LLM-именование
