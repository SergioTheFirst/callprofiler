...
- **Root cause (non-obvious):** `restore_backup()` подменяет основной файл БД атомарно
  ...
  файл. Если `to_path` раньше был живой БД в WAL-режиме (штатно — каждое соединение через
  `ConnectionFactory` включает `journal_mode=WAL`, `.claude/rules/db.md`), рядом лежат
  `<to_path>-wal`/`<to_path>-shm` со СТАРЫМИ, ещё не влитыми в основной файл данными. Эти
  sidecar-файлы принадлежат СТАРОМУ (только что выброшенному) содержимому `to_path`, но остаются
  на диске нетронутыми. Первое же открытие свежевосстановленного файла (в т.ч. верификация
  счётчиков строк внутри самого `restore_backup`) реплеит их поверх — восстановленные данные из
  бэкапа молча перекрываются старыми кадрами WAL. Симптом: ожидали N строк из бэкапа, получили
  число строк из ПЕРЕЗАПИСАННОЙ "живой" БД — то есть restore выглядит успешным (файл открывается,
  quick_check проходит), просто содержит не те данные.
- **Почему не ловилось раньше:** тест `test_restore_overwrite_snapshots_current_state_first`
  (существовал до этой сессии) реально ловит баг — но заявленная базовая линия «1459 passed»
  (2026-08-08) была не полной верификацией реальной установкой (см. запись ниже про extras) —
  похоже, тест либо не запускался в среде, где WAL-хвосты успевали материализоваться на диске
  тем же образом, либо расхождение просто не заметили. НЕ полагаться на «зелёные тесты» без
  прогона реальной командой `pip install` + `pytest` с нуля.
- **Fix:** `restore_backup()` удаляет `<to_path>-wal` и `<to_path>-shm` (если есть) сразу после
  `os.replace`, ДО верификации счётчиков — restored-файл гарантированно стартует со свежим,
  пустым WAL при следующем открытии. Regression: существующий тест (не вакуумный — падал без
  фикса с тем самым мисматчем, проходит с фиксом; новый тест не понадобился).
- **Blast radius:** высокий — это ровно тот гейт (T-20, P-OPS-01), который должен защищать
  T-05 migrations от порчи БД. Любой ручной restore на боевых данных БЕЗ этого фикса тихо
  откатывал бы БД к состоянию непосредственно перед restore, а не к содержимому бэкапа.
- **Status:** RESOLVED (2026-08-21).

✅ **T-01/cloud extras: `pip install -e ".[cloud]"` реально ставил полный ML-стек — pip extras не могут исключить `dependencies`** (2026-08-21)
- **Root cause (non-obvious):** в Python packaging extras (`[project.optional-dependencies]`)
  ТОЛЬКО ДОБАВЛЯЮТ пакеты к обязательным `dependencies` — не существует механизма «извлечь
  extra X, но не базу». `torch`/`faster-whisper`/`pyannote.audio`/`soundfile`/`librosa` жили в
  безусловных `dependencies` (нужны боксу, `pip install -e .`). Extra `cloud` (введён 2026-08-08
  для дешёвого облачного прогона) добавлял `pytest`/`fastapi`/etc, но не мог УБРАТЬ ML-стек —
  само существование extra было бессильно против базового списка. `pip install -e ".[cloud]"`
  ставил ~150 пакетов (несколько ГБ), а не заявленные ~40МБ.
- **Почему не ловилось:** заявленное «1415 passed, 7 skipped, 0 failed» было измерено
  СИМУЛЯЦИЕЙ отсутствия torch (заглушка/broken import), а не реальной установкой через саму
  команду `pip install -e ".[cloud]"` в чистом окружении. Симуляция проверяет «код переживёт
  отсутствие torch», но не проверяет «команда установки реально не ставит torch» — это два
  разных утверждения, и только первое было протестировано. Нашлось на первом реальном
  автономном прогоне в облаке (эта сессия), когда установка заняла минуты и явно потянула
  CUDA-пакеты.
- **Fix:** ML/audio-пакеты вынесены в отдельный extra `full` (`pip install -e ".[full]"` —
  боевой бокс/dev). `cloud` остался лёгким намеренно (`dependencies` + `dev`-подобный набор).
  README.md/AGENTS.md/docs/TESTING.md обновлены на `.[full]`/`.[dev,full]`. Перепроверено реальной
  установкой в чистый venv: до фикса torch стоит (150 пакетов), после — отсутствует (39 пакетов).
- **Урок:** «suite зелёный с заглушкой X» ≠ «команда установки реально не ставит X». Для
  claim'ов вида «облако дёшево / без стека Y» верифицировать РЕАЛЬНОЙ командой в чистом venv,
  не симуляцией отсутствия импорта.
- **Status:** RESOLVED (2026-08-21).
...
- **Root cause (non-obvious):** `pyannote_runner.load()` начинался с `if self.pipeline is not None: return`,
  ...
  строка выглядит разумно. Ломает их `orchestrator._diarize_batch`: он группирует звонки по `ref_audio`
  (`by_ref`) и зовёт `load()` на каждую группу, но между группами runner НЕ выгружает — выгрузка только
  в `_unload_models()` после всей Фазы 2 (это осознанная perf-оптимизация 2026-06-06, ко-резидентность
  GigaAM+pyannote). Итог: для 2-й и последующих ref-групп ранний return пропускал построение эмбеддинга,
  и эталон оставался от ПЕРВОГО профиля → голос владельца A назначался OWNER в звонках B.
- **Почему не ловилось:** симптом не крэш, а тихо неверные роли. Никакой ошибки в логах; на одном
  профиле (текущее состояние бокса — только `me`) дефект вообще не проявляется. Тест
  `test_diarize_batch_loads_once_for_group` даже ЗАКРЕПЛЯЛ «load один раз на группу» — правильное
  свойство, но проверял его на фейке без эмбеддинга, поэтому утечку не видел.
- **Fix (важно — не тот, что напрашивается):** unload/reload на каждую ref-группу НЕ нужен. Pipeline
  pyannote от `ref_audio` не зависит — зависит только эмбеддинг. Поэтому `load()` сделан идемпотентным
  ПО ОТПЕЧАТКУ ref (`_ref_fingerprint` = abspath+size+mtime_ns): тот же ref → no-op; другой ref →
  модель не трогаем, пересобираем только `ref_embedding`; pipeline отсутствует → полная загрузка.
  Плюс `unload()` чистит отпечаток, `FileNotFoundError` поднят выше раннего return, `diarize()` без
  эмбеддинга деградирует в UNKNOWN, а `_diarize_batch` перед каждой группой сверяет отпечаток
  **fail-closed** (`getattr(..., None)`, не `getattr(..., expected)` — с дефолтом сторож бесшумно
  исчезает для любого runner'а без атрибута).
- **Regression:** `tests/test_pyannote_ref_isolation.py` (8 тестов, оба порядка A→B/B→A + пропуск
  группы при рассогласовании). Реальная диаризация двух профилей — проверка на боксе.
- **Status:** RESOLVED код (2026-08-08), 1317 passed/3 skipped.
