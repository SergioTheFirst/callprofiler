...
## Серия sintezdiharea 2026-08-22: вертикальные срезы T-06…T-25 в режиме оркестрации (2026-08-22)

Решение владельца: после отложенного BS-research продолжать задачи `docs/sintezdiharea.md` «без остановки
до конца» в режиме оркестрации (Workflow: Opus implement → Haiku/Sonnet test-gate → Sonnet review → fix
loop) с адаптивным распределением (мелкие/T3-срезы — лично), ponytail ultra. **Принцип срезов:** из
каждой P0/P1-задачи взят вертикальный срез, закрывающий её acceptance по существу, без state-machine
/outbox/executor-фреймворков (один процесс, один writer): T-07 backoff+терминальный гард; T-06 purge по
introspection + карантин файлов; T-08 атомарные карточки/архив; T-12 VRAM-барьер; T-11 ASR-покрытие;
T-14 промпт v002 (envelope, владелец из профиля, клип); T-15 строгий анализ (нет выдуманных анализов);
T-16 идемпотентные promises/events; T-17 токен не логируется; T-21 doctor backup/dead-letters; T-22 коды
выхода; T-23 удаление психометрики без инструмента; T-24 матрица изоляции; T-19 единая RiskPolicy; T-09
`_fail()` + loader→transcribed; T-25 — чек-лист бокса. Не сделано осознанно (YAGNI до второго writer'а
/внешних потребителей): lease/attempts/reconciler (T-07), outbox-worker и подписанные callback'и (T-17),
GpuCoordinator как state machine (T-12), model manifest (T-11), step-executor как фреймворк (T-09),
structured JSON-логи (T-21). **Урок оркестрации (повторился в T-06/T-11/T-14/T-15):** отчёт агента и
зелёный узкий gate ≠ доказательство — агенты оставляли ручные списки там, где просили introspection,
вызывали несуществующие API (`Config.load()`), писали тест-файлы в корень с битым путём, забывали
литералы; каждый дифф перечитывался и чинился лично, full-suite гонялся локально перед push.
Следующий шаг — канарейка на боксе (`docs/ops/box-canary-checklist.md`).

## BS-index → «надёжность обязательств» (CR) + индекс уверенности (CI): исследование завершено планом T-26 (2026-08-22)

Запрос (`docs/routines/bs-research-fable.md`): пересмотреть с нуля механизм «надёжности речи/поведения
абонента» и добавить калиброванный индекс уверенности 1–100. Результат — `docs/research/bs/90-execution-
plan.md` (R-01…R-30; код не писался). **WHY (из ledger, `docs/research/bs/claims-ledger.md`):** (1) в проде
`bs_index = 20·min(contradictions/total_calls,1)` — 4 из 5 членов `v1_linear` структурно ≡ 0
(`graph/repository.py:399-402` схлопывает vagueness/blame_shift/emotion_spike в `'fact'`,
`broken_promise` никто не производит; C-04/D1), `who='UNKNOWN'` у всех structured_facts (D2), verbatim-
гейт цитат работает только в `graph-replay`, не в боевом пути (`orchestrator.py:994`, `enricher.py:276`,
`uow.py:8`; D3), «калибровка» `bs_thresholds` ранговая (10% «critical» при любых данных; D6). (2) «BS»
как конструкт нефальсифицируем на этих данных: текстовые cues к обману d≈0.10 (DePaulo 2003), люди 54%
(Bond & DePaulo 2006); противоречия и хеджи — норма честной речи (C-01…C-03). **Решение:** `v1_linear`
заморозить как LEGACY (aggregator продолжает писать колонку — инвариант graph.md; потребители
отключаются), ввести CR = частота исполнения обязательств контакта в диаде (Beta-binomial по
`promise_outcomes` с измеряемой точностью ярлыков, весом качества ролей, проверяемым затуханием) и CI =
масса posterior в окне ширины 0.2 вокруг медианы (семантика «вероятность, что показанное верно»,
проверка reliability diagram); UI: факты-исходы всегда, описательная фраза прошедшего времени («из
известных обещаний выполнял большинство») при CI≥50, число — только после ≥150 ✓/✗-исходов владельца;
admiralty A–F, trait-паттерны профайлера, `bs_score`-совет — удалить. Лингвистические сигналы
(противоречия, хеджи) — только бокс-эксперименты с записанным правилом решения; маркеры уверенности
сняты враждебной рецензией как детектор речевого паттерна (C-09 retracted). Большинство контактов
получат «данных мало» — это главный эффект честности (S-2). **Вердикт владельца (2026-08-22): ОТЛОЖЕНО, не применять.** Причины: план перечёркивал имеющиеся
наработки вместо развития; зависел от калибровки на реальной БД (adjudication); не дал «индекс из того,
что есть» для нового пользователя без истории. Материалы остаются в `docs/research/bs/` как справка;
T-26 в `sintezdiharea.md` не добавляется. Урок для будущих исследований — memory
`research-build-on-existing`: сначала формула из существующих сигналов для new-user случая, потом
уточнения; «заморозить и заменить» — не дефолт.
