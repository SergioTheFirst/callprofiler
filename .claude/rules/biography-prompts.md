...
- Output:
  ...
  - Прозаические (p6 chapter, p8 edited chapter, p9 yearly): **только markdown**,
    без JSON, без «вот ваша глава».
- Язык вывода: **русский**. Имя владельца — Сергей Станиславович Медведев.
- **Имена в тексте — живое письмо:** используй так, как звучит в материале.
  Только «Медведев Сергей» — однозначно владелец. Просто «Сергей» в диалоге
  может быть другой человек — уточнять по контексту. В первом упоминании:
  уточни роль или контекст: «Сергей (директор проекта)» vs. «Сергей (владелец)».
...
- `transcript` (клипается до 12000 символов: 6000 head + 6000 tail)
...
### Pass 9 — Yearly Summary (`build_yearly_summary_prompt`)

**Input:**
- `year`: int
- `chapters`: список глав года (period, title, theme, excerpt ≤400 символов)
- `top_arcs` (≤12), `top_entities` (≤15)

**Output:** **только markdown**, 3-5 абзацев без подзаголовков.

**Стиль:** Довлатов — спокойно, без морали, с иронией и эмпатией. Рефлексия
без «выводов для жизни». Сквозные мотивы года, а не пересказ глав.

**Хранение:** `bio_books` с `book_type="yearly_summary"`.
