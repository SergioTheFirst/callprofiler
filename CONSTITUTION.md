...
## Статья 19. Память проекта — Continuity Ledger

### 19.1. Принцип: непрерывность знаний

Любой разработчик или AI-агент может открыть репозиторий и мгновенно понять:
- что является целью и критерием успеха;
- какие жёсткие ограничения никогда не нарушаются;
- что сделано, что делается сейчас, что делать дальше.

Это требует **обязательного ведения двух журналов**: `CONTINUITY.md` и `CHANGELOG.md`.

### 19.2. CONTINUITY.md — Continuity Ledger

Единственный файл, который нужно прочитать перед любой работой с проектом.
**Перезаписывается** при каждом обновлении (не приписывается).

Канонический формат (Continuity Ledger):
```markdown
# CONTINUITY.md — Continuity Ledger

> Canonical session briefing. Facts only, no transcripts.

**Goal (incl. success criteria):**
- [Цель + измеримый критерий завершения]

**Constraints/Assumptions:**
- [Неизменяемые ограничения: стек, GPU, DB, etc.]

**Key decisions:**
- [Архитектурные решения с кратким WHY]

**State:**

Done:
- [Что завершено: описание + commit hash]

Now:
- [Текущее состояние: idle / в работе]

Next:
- [Следующий шаг / следующая Фаза]

**Open questions (UNCONFIRMED):**
- [Нераскрытые вопросы, требующие проверки в коде]

**Working set (files/ids/commands):**
- [Релевантные файлы, ID, команды для текущего шага]
```

**Правило:** перед `git push` — обновить CONTINUITY.md. Раздел State.Now → State.Done; заполнить State.Next.

### 19.3. CHANGELOG.md — история изменений
...
### Changed — Title (YYYY-MM-DD)
- что изменено
```

**Правило:** каждый коммит с кодом → запись в Unreleased.

### 19.4. Сессионный протокол

При начале:
```
→ прочитать CONTINUITY.md
→ вывести: "State: [Now] / Next: [Next]"
```

При завершении:
```
→ обновить State в CONTINUITY.md (Done/Now/Next)
→ добавить запись в CHANGELOG.md
→ pytest tests/ -q
→ git add CONTINUITY.md CHANGELOG.md && git commit && git push origin main
```

### 19.5. Обязательность

- **Нарушение этой статьи = нарушение Конституции.**
- Каждый коммит в main сопровождается обновлением CONTINUITY.md.
- Пропуск журналов = потеря контекста = нарушение Ст.19.1.
