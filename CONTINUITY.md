...
Методология `vozrast.md`. Карта: `.claude/rules/insight.md` («Возраст-стиль»). 4-й сигнальный
класс, ОТДЕЛЬНЫЙ от marker/relation/LLM `age_estimate.py` (своя таблица `contact_age_style`,
свой пайплайн `insight/age_style/`, никогда не пишет/не читает `contact_age_estimates`).
Поток: no-ML фичи (лексика/морфосинтаксис/читаемость/разнообразие) → z внутри популяции юзера →
8 вероятностных таблиц (vozrast.md §4.2, дословно) → взвешенный линейный пул по группам G1-G6
(деконфликт коррелирующих измерений разнообразия) → год рождения (анкор — средний год звонков
контакта, НЕ дата прогона) → confidence (sigmoid). CLI `age-style --user X [--stale-only]`,
watcher-autofit (stale_only, безмодельно — не конфликтует с ASR-GPU), дашборд-секция
«Возраст (стиль)» (group-бары/★-доверие/топ-вклады) + `POST /api/tools/age-recompute?contact_id=`
(полная популяция синхронно, без GPU/LLM — z-score популяционный). Security-review: APPROVED,
0 findings. Попутный баг (Ф0): маркер «внуки» ложно срабатывал на «Внуково» — исправлено.
727 passed/2 skipped (полный `tests/`). **Не проверено на реальной БД/боксе.**
