...
🎯 **АКТИВНЫЙ ПРИОРИТЕТ: Stage-1 audio→БД** (biography-дизайн ОТЛОЖЕН). План — в
`.claude/rules/decisions.md` («Stage-1 (audio→БД) — НЕМЕДЛЕННЫЙ приоритет»). Suть:
гнать `enable_llm_analysis:false` + `enable_diarization:false` = чистый audio→текст→БД
без llama-server/pyannote. **Единственный необходимый код-фикс:** `process_batch` не
терминализует transcribe-only звонки (Phase4 deliver гейт `if stage<3: continue`) → они
застревают status=`transcribing`/stage2 и реклаймятся вечно. Фикс: терминальный статус
**`transcribed`** в Pass C при выключенном анализе + `get_stalled_calls` его исключает +
dashboard stage-map. Роли/LLM/граф/биография — ПОТОМ (роли позже без ре-ASR: наложение
pyannote-спанов на `transcripts.start_ms/end_ms`).
...
**Next (Stage-1):**
- [реализуется] Терминальный статус `transcribed` для transcribe-only batch: фикс `process_batch`
  (Pass C/Phase4) + `get_stalled_calls` исключает `transcribed` + dashboard stage-map. TDD (RED→GREEN).
- На боксе: `enable_llm_analysis:false`+`enable_diarization:false`, файлы в `C:\calls\in`, `startprocess.bat`
  (watch). Цель: все audio → транскрипты в БД + `.txt`, звонки терминализуются в `transcribed`, дашборд не врёт.
- ПОТОМ (Stage-2, отдельно): поднять llama-server → bulk-enrich над `status='transcribed'`; затем роли,
  граф v2, профили, биография; biography→analysis resilience-дизайн.
