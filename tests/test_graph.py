...
def test_upsert_entity_metrics(setup):
    ...
    m = grepo.get_entity_metrics(eid, "u1")
    ...
    m2 = grepo.get_entity_metrics(eid, "u1")
...
def test_full_recalc_entity_not_found_raises(setup):
    ...
    with pytest.raises(ValueError, match="not found"):
        agg.full_recalc_from_events(99999, "u1")
...
def test_full_recalc_idempotent(setup):
    ...
    r1 = agg.full_recalc_from_events(eid, "u1")
    r2 = agg.full_recalc_from_events(eid, "u1")  # second call must produce identical result
...
def test_resolver_execute_merge_basic(setup):
    ...
    m = grepo.get_entity_metrics(canonical_id, "u1")
    assert m is not None
